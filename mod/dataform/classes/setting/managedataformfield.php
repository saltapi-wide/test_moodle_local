...
class managedataformfield extends \admin_setting {
    /**
    ...
    public function output_html($data, $query = '') {
        ...
        foreach ($allplugins as $plugin => $unused) {
            ...
            if (isset($enabled[$plugin])) {
                ...
                $hideshow .= "<pix src=\"" . $OUTPUT->pix_url('t/hide') . "\" class=\"iconsmall\" alt=\"$strdisable\" /></a>";
                $isenabled = true;
                ...
                if (isset($available[$plugin])) {
                    ...
                    $hideshow = "<a href=\"$aurl\">";
                    $hideshow .= "<pix src=\"" . $OUTPUT->pix_url('t/show') . "\" class=\"iconsmall\" alt=\"$strenable\" /></a>";
                    $isenabled = false;
            ...
            if ($isenabled) {
                if ($updowncount > 1) {
                    ...
                    $updown .= "<a href=\"$aurl\">";
                    $updown .= "<pix src=\"" . $OUTPUT->pix_url('t/up') . "\" alt=\"$strup\" class=\"iconsmall\" /></a>&nbsp;";
                } else {
                    $updown .= "<pix src=\"" . $OUTPUT->pix_url('spacer') . "\" class=\"iconsmall\" alt=\"\" />&nbsp;";
                }
                if ($updowncount < $plugincount) {
                    $aurl = new \moodle_url($url, array('action' => 'down', 'plugin' => $plugintype));
                    $updown .= "<a href=\"$aurl\">";
                    $updown .= "<pix src=\"" . $OUTPUT->pix_url('t/down') . "\" alt=\"$strdown\" class=\"iconsmall\" /></a>";
                } else {
                    $updown .= "<pix src=\"" . $OUTPUT->pix_url('spacer') . "\" class=\"iconsmall\" alt=\"\" />";
                }
                ++$updowncount;
