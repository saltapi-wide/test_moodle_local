        ...
        if (is_array($structure)) {
            ...
            foreach ($structure as $element) {
                ...
                if ($res) {
                    ...
                    trainingsessions::updatefirst($dataobject->firstaccess, 1);
                    trainingsessions::updatelast($dataobject->lastaccess, 1);
            ...
            if (!empty($structure->name) && empty($config->showsectionsonly) || (!empty($config->showsectionsonly) && !empty($structure->subs))) {
                ...
                $worksheet->write_string($thisrow, $col, $convertedelapsed, $xlsformats['a']);
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            if ($dataformats[$i] == 'd') {
                if ($data[$i]) {
                    ...
                    if ($celldata !== null && $celldata !== '') {
                        $worksheet->write_string($row, $i, $celldata, null);
                    }
                    continue;
                } else {
                    continue;
