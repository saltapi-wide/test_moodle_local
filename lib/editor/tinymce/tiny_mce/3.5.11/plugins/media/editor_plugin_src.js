	...
	tinymce.create('tinymce.plugins.MediaPlugin', {
		init : function(ed, url) {
			...
			ed.onInit.add(function() {
				// Display "media" instead of "pix" in element path
		...
		/**
		 * Converts the JSON data object to an pix node.
		 */
		dataToImg : function(data, force_absolute) {
			...
			img = self.editor.dom.create('img', {
				...
				hspace : data.hspace,
				vspace : data.vspace,
				src : self.editor.theme.url + '/pix/trans.gif',
		...
		/**
		 * Converts a tinymce.html.Node video/object/embed to an pix element.
		 *
		 * The video/object/embed will be converted into an image placeholder with a JSON data attribute like this:
		 * <pix class="mceItemMedia mceItemFlash" width="100" height="100" data-mce-json="{..}" />
		 *
		...
		objectToImg : function(node) {
			...
			img = new Node('img', 1);
			img.attr({
				src : this.editor.theme.url + '/pix/trans.gif'
			});
