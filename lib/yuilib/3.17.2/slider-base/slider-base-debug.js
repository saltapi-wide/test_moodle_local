...
Y.SliderBase = Y.extend( SliderBase, Y.Widget, {
    ...
    _initThumbUrl: function () {
        if (!this.get('thumbUrl')) {
            ...
            // <pix src="/path/to/build/slider-base/assets/skins/sam/thumb-x.png">
    ...
    /**
     * Thumb template that will contain the thumb image and shadow. &lt;pix>
     * tags are used instead of background images to avoid a flicker bug in IE.
     * {placeholder}s are used for template substitution at render time.
     *
     * @property THUMB_TEMPLATE
     * @type {String}
     * @default &lt;span class="{thumbClass}" tabindex="-1">&lt;pix src="{thumbShadowUrl}" alt="Slider thumb shadow" class="{thumbShadowClass}">&lt;pix src="{thumbImageUrl}" alt="Slider thumb" class="{thumbImageClass}">&lt;/span>
     */
